...
Sends an email to one or more recipients based on the template text specified by the path and replacing variables in the template text using the email parameters. See [Creating an Email Template]({{ page.url }}#creating_an_email_template) section below for an example template.
